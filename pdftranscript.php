...
$sql2 = "SELECT * FROM studentstable  WHERE student_id='$student_id'";
...
$sql3 = "SELECT * FROM course  WHERE coursename='$course_id'";
...
$sqlf = "SELECT * FROM marks  WHERE course_id=$course_id AND student_id=$student_id AND year=$year AND term='$term' ";
...
while($users5 = mysqli_fetch_array($resultsetmarks, MYSQL_ASSOC)){
        $maths=$users5['maths'];
        $english=$users5['english'];
        $kiswahili=$users5['kiswahili'];
        $social_studies=$users5['social_studies'];
        $science=$users5['science'];
...
$pdf->Cell(0,10,'Mathematics                                                                                        '.$maths ,0,1);
$pdf->Cell(0,10,'English                                                                                                 '.$english ,0,1);
$pdf->Cell(0,10,'Kiswahili                                                                                              '.$kiswahili ,0,1);
$pdf->Cell(0,10,'Chemistry                                                                                            '.$social_studies ,0,1);
$pdf->Cell(0,10,'Physics/Biology                                                                                  '.$science ,0,1);

$pdf->Cell(0,10,'Average                                                                                                '.$average ,0,1);

$pdf->Ln(4); //break  
$pdf->Line(0, 150, 210, 150);  //Set the line
$pdf->Ln(4); //break  
...
$pdf->SetFont('','');
$pdf->Ln();
$pdf->Cell(100,10,"Pass grade  ".$classgrade,1,0,'L',0);
...
$pdf->SetFont('','i');
$pdf->Write(10,'NB: This is not a certifcate');
$pdf->SetTextColor(0,0,0);
//nore text
$pdf->Ln();
$pdf->SetTextColor(0,0,0);
